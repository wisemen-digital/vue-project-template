...
export type RouteRecordTyped = Omit<RouteRecordRaw, 'children' | 'name'> & {
...
function setupRouteMiddlewareIncterceptor(router: Router): void {
  const hasMiddleware = (to: RouteRecordNormalized): to is RouteRecordNormalized & {
    meta: {
      middleware: RouteMiddleware[]
    }
  } => to.meta.middleware != null
  
  const handleRouteMiddlewares = async (to: RouteLocationNormalized, from: RouteLocationNormalized): Promise<RouteLocationRaw | undefined> => {
    const middlewares = to.matched
      .filter(hasMiddleware)
      .flatMap(({ meta }) => meta.middleware)
