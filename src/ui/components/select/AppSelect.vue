...
const hasDefaultSlotContent = computed<boolean>(() => {
  ...
  return (
    defaultSlot != null
    && defaultSlot[0].children != null
    && (defaultSlot[0].children.length as number) > 0
  )
})
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <ComboboxButton
          class="absolute left-0 top-0 flex h-full w-full items-center justify-end bg-transparent p-3"
        >
          <AppIcon
            class="h-3 w-3"
            icon="chevronDown"
