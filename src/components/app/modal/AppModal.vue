...
const props = withDefaults(
  ...
  {
    modalClasses: 'left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2',
    modalId: null,
  },
)
...
const animateWithViewTransitionsApi = computed<boolean>(() => {
  const activatorElement = document.querySelector(`#${props.modalId}`) ?? null

  // @ts-expect-error - TS doesn't know about startViewTransition
  return props.modalId != null && document.startViewTransition != null && activatorElement !== null
})

function animateIn(): void {
  const button = document.querySelector(`#${props.modalId}`) as HTMLElement
